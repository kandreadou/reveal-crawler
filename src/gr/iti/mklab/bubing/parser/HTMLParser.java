...
/**
 * This is the same as {@link it.unimi.di.law.bubing.parser.HTMLParser} from the 0.9.5 sources.
 * The only difference is that some access modifiers have been changed from private to protected
 * to facilitate extending this class without duplicating the whole code.
 *
 * @author kandreadou
 */
...
public class HTMLParser implements Parser {
     ...
     */
    public HTMLParser(final HashFunction hashFunction, final boolean crossAuthorityDuplicates, final int bufferSize) {
    ...
    public HTMLParser(final HashFunction hashFunction, final boolean crossAuthorityDuplicates) {
        this(hashFunction, crossAuthorityDuplicates, CHAR_BUFFER_SIZE);
    }
    ...
    public HTMLParser(final String messageDigest, final String crossAuthorityDuplicates) throws NoSuchAlgorithmException {
        this(BinaryParser.forName(messageDigest), Boolean.parseBoolean(crossAuthorityDuplicates));
    }
     ...
     */
    public HTMLParser() {
        this((HashFunction) null, false, 0);
    }
    ...
    public HTMLParser clone() {
        return new HTMLParser(digestAppendable == null ? null : digestAppendable.hashFunction, crossAuthorityDuplicates, buffer.length);
    }

    @Override
    public HTMLParser copy() {
        return clone();
    }
    ...
    public static void main(String arg[]) throws IllegalArgumentException, IOException, URISyntaxException, JSAPException, NoSuchAlgorithmException {
        ...
        SimpleJSAP jsap = new SimpleJSAP(HTMLParser.class.getName(), "Produce the digest of a page: the page is downloaded or passed as argument by specifying a file",
                new Parameter[]{
        ...
        final HTMLParser htmlParser = new HTMLParser(BinaryParser.forName(digester), crossAuthorityDuplicates, charBufferSize);
